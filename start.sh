...
if [ $deploymentMode = serverless ]; then
   kubectl apply --filename knative/serving-crds.yaml
   kubectl apply --filename knative/serving-core.yaml
   kubectl apply --filename knative/release.yaml
   echo "😀 Successfully installed Knative"
fi

# Install Cert Manager
kubectl apply --validate=false -f cert/cert-manager.yaml
...
helm install kserve-crd helm-chart-kserve-crd-v0.10.1.tgz -n kserve  
helm install kserve-runtime helm-chart-kserve-v0.10.1.tgz -n kserve  
